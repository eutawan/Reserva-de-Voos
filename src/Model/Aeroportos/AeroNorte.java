...
public class AeroNorte extends Aeroporto {
    public AeroNorte(String nome, String localizacao, int qntMaxReservas) {
        super(nome, localizacao, qntMaxReservas);
    }

    public void exbirInfo() {
        System.out.println("Nome do aeroporto: " + nome);
        System.out.println("Localização: " + localizacao + ". Na América do norte.");
    ...
    public void fazerReserva(Reserva reserva){
        for (int i = 0; i < reserva.assentosReservados; i++){
            if (listaDeReservas[i] == null){
                ...
                System.out.println("Voo no Aeroporto da América do Norte realizado com sucesso!");
            }
        }
    }
}
